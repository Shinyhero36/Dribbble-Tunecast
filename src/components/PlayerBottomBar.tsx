...
import { Playback } from "./PlayerCard";
      ...
      <div className="flex flex-row justify-between">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <Control className="hidden sm:block">
            <ShuffleIcon className="h-5 w-5 text-gray-400 sm:h-6 sm:w-6" />
          </Control>
          {/* Previous - Play/Pause - Next */}
          <div className="flex items-center justify-between gap-1 pr-4 sm:pr-0">
            {/* Previous */}
            <Control>
              <BackwardIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            </Control>
            ...
            {/* Next */}
            <Control>
              <ForwardIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            </Control>
          </div>
          {/* Repeat */}
          <Control className="hidden sm:block">
            <RepeatIcon className="h-5 w-5 text-gray-400 sm:h-6 sm:w-6" />
          </Control>
        </div>
        ...
        <div className="hidden items-center justify-between gap-4 pr-10 sm:flex">
          <div className="items-centeer flex justify-center gap-2">
            <Control>
              <HeartIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            </Control>
            <Control>
              <PlusCircleIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            </Control>
          </div>
          <div className="flex items-center justify-center gap-2">
            {volume === 0 ? (
              <SpeakerMuteIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            ) : (
              <SpeakerIcon className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
            )}
            <input
              type={"range"}
              min={0}
